...
class db:
    def __init__(self):
        ...
        self.cc = pd.read_csv(r'C:\Users\Mateusz\Documents\Olek\Mod_16\Mod_16_Zad_2\db\country_codes.csv',index_col=0)
        self.customers = pd.read_csv(r'C:\Users\Mateusz\Documents\Olek\Mod_16\Mod_16_Zad_2\db\customers.csv',index_col=0)
        self.prod_info = pd.read_csv(r'C:\Users\Mateusz\Documents\Olek\Mod_16\Mod_16_Zad_2\db\prod_cat_info.csv')
    ...
    def transation_init():
        ...
        src = r'C:\Users\Mateusz\Documents\Olek\Mod_16\Mod_16_Zad_2\db\transactions'
...
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[
                            ...
                            dcc.Tab(label='Kanały', value='tab-3')
                            ]),
...
def tab2_barh_prod_subcat(chosen_cat):
    ...
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['prod_cat']==chosen_cat)].pivot_table(index='prod_subcat',columns='Gender',
              values='total_amt',aggfunc='sum').assign(_sum=lambda x: x['F']+x['M']).sort_values(by='_sum').round(2)
...
def tab3_scatter_plot(placeholder):
    dff = df.merged.copy()
    dff["weekday"] = dff["tran_date"].apply(lambda x: x.weekday())
    grouped = dff[dff['total_amt']>0].groupby([pd.Grouper(key='weekday'), 'Store_type'])['total_amt'].sum().round(2).unstack()
    data=[]
    for col in grouped.columns:
        data.append(go.Scatter(x=grouped.index,y=grouped[col],name=col,hoverinfo='text'))
